...
- **Busy detection** uses the public event stream: `session.status` (`busy`/`retry`/`idle`), `session.execution.started/succeeded/failed/interrupted`, `session.idle`, streaming deltas and tool activity. Prompts waiting for you come from `permission.asked` / `permission.replied` and `question.asked` / `question.replied` / `question.rejected`. Every event is matched against the plugin's own location, so a busy session in another project does not light up your pill.
