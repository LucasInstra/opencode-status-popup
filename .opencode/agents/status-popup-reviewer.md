...
2. Option plumbing: every key parsed in `src/config.ts` (`enabled`, `mode`, `word`, `typeMs`, `position`, `freshSeconds`, `idleSeconds`, `errorHoldSeconds`, `mark`, `trayIdleStatic`, `shellPath`) must reach the place that uses it — compare `PopupConfig` against the `HostSettings` built in `src/index.ts`, the `HostSettings`/`buildArgs`/`matches` trio in `src/host.ts`, the `host/popup.ps1` parameters and the README options table. An option that is parsed and documented but never forwarded is a bug, not a doc nit.
