...
2. `npm run typecheck`
3. `npm test` — expect 10 test files / 62 tests with the smoke file skipped (60 passed, 2 skipped). Counts drift between releases; report actual vs expected.
...
5. `npm pack --dry-run --json` — the tarball file list (21 files as of 0.2.3) must cover `index.ts`, `tui.ts`, `src/**`, `host/**` (every `.ps1` plus `TrayIcon.cs`), `README.md`, `LICENSE`, and nothing the runtime does not need.
