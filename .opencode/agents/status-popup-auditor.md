...
2. **Command surfaces**: the five `/popup-*` names are identical on the server side (`src/index.ts`), the palette side (`src/tui.ts`, which must stay palette-only and never register a `slash`), the README "Commands" section, and the tests (`tests/tui.test.ts`, `tests/smoke.test.ts`): `popup-window|tray|toggle|reset` drive the renderer choice and are the ones the `popup_mode` tool exposes; `popup-static` toggles the tray idle and deliberately has no tool.
